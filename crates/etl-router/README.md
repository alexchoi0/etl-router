...
Main binary for the ETL Router control plane.

## Overview

This crate provides the entry point for running an ETL Router node. It orchestrates all the components needed for a router cluster member: gRPC services, Raft consensus, GraphQL API, and service registry.
...
- `etl-grpc` - gRPC service implementations
- `etl-raft` - Raft consensus
- `etl-config` - Configuration loading
