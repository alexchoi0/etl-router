    ...
    async fn push_records(
        ...
        let output = async_stream::try_stream! {
            while let Some(req) = stream.message().await? {
                match req.msg {
                    Some(conveyor_proto::source::push_request::Msg::Batch(batch)) => {
                        ...
                        if buffer.should_backpressure(&source_id).await {
                            yield PushResponse {
                                msg: Some(conveyor_proto::source::push_response::Msg::Backpressure(
                                    BackpressureSignal {
                                        pause: true,
                        ...
                        yield PushResponse {
                            msg: Some(conveyor_proto::source::push_response::Msg::Ack(BatchAck {
                                batch_id: batch.batch_id.clone(),
                                record_acks,
                            })),
                        };
                    }
                    Some(conveyor_proto::source::push_request::Msg::Heartbeat(_hb)) => {
                        let buffer = buffer_manager.read().await;
                        let source_id = "default";
                        let credits = buffer.available_credits(source_id).await;

                        yield PushResponse {
                            msg: Some(conveyor_proto::source::push_response::Msg::Credits(CreditGrant {
                                credits,
