...
use conveyor_raft::{RaftNode, RouterCommand, SidecarLocalService, SidecarStageTarget};
    ...
    async fn compute_pipeline_assignments(
        ...
        for (pipeline_id, pipeline) in &state.pipelines {
            ...
            for svc in local_services {
                stage_assignments.push(conveyor_proto::sidecar::StageAssignment {
                    stage_id: svc.service_name.clone(),
                    target: Some(conveyor_proto::sidecar::stage_assignment::Target::LocalEndpoint(
                ...
                for svc in &other_sidecar.local_services {
                    if !local_services.iter().any(|ls| ls.service_name == svc.service_name) {
                        ...
                        stage_assignments.push(conveyor_proto::sidecar::StageAssignment {
                            stage_id: svc.service_name.clone(),
                            target: Some(
                                conveyor_proto::sidecar::stage_assignment::Target::RemoteSidecar(
                                    conveyor_proto::sidecar::RemoteSidecar {
                ...
                if let Some(sidecar) = state.sidecars.get(&sidecar_id) {
                    for (pipeline_id, stages) in &sidecar.assigned_pipelines {
                        let assignment = PipelineAssignment {
                            ...
                            stages: stages
                                .iter()
                                .map(|s| conveyor_proto::sidecar::StageAssignment {
                                    stage_id: s.stage_id.clone(),
                                    target: match &s.target {
                                        SidecarStageTarget::Local { endpoint } => Some(
                                            conveyor_proto::sidecar::stage_assignment::Target::LocalEndpoint(
                                                endpoint.clone(),
                                            ),
                                        ),
                        ...
                        let event = PipelineAssignmentEvent {
                            event_type: conveyor_proto::sidecar::EventType::Assigned as i32,
