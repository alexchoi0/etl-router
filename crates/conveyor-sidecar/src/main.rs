...
use conveyor_proto::sidecar::sidecar_data_plane_server::SidecarDataPlaneServer;

use conveyor_sidecar::config::SidecarConfig;
use conveyor_sidecar::discovery::GrpcReflectionDiscovery;
use conveyor_sidecar::routing::{RoutingTable, LocalRouter, RemoteRouter};
use conveyor_sidecar::cluster_client::{ClusterRegistration, HeartbeatLoop};
use conveyor_sidecar::SidecarDataPlaneImpl;

#[tokio::main]
async fn main() -> Result<()> {
    tracing_subscriber::registry()
        .with(tracing_subscriber::EnvFilter::try_from_default_env()
            .unwrap_or_else(|_| "conveyor_sidecar=info,tower_http=debug".into()))
