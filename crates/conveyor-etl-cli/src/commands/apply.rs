...
use clap::Args;
use conveyor_etl_dsl::manifest::{AnyManifest, ResourceKind};
...
fn load_manifest_file(path: &PathBuf) -> Result<AnyManifest> {
    ...
    let manifest = match kind_probe.kind {
        ResourceKind::Source => {
            let m: conveyor_etl_dsl::SourceManifest = serde_yaml::from_str(&content)?;
            AnyManifest::Source(m)
        }
        ResourceKind::Transform => {
            let m: conveyor_etl_dsl::TransformManifest = serde_yaml::from_str(&content)?;
            AnyManifest::Transform(m)
        }
        ResourceKind::Sink => {
            let m: conveyor_etl_dsl::SinkManifest = serde_yaml::from_str(&content)?;
            AnyManifest::Sink(m)
        }
        ResourceKind::Pipeline => {
            let m: conveyor_etl_dsl::PipelineManifest = serde_yaml::from_str(&content)?;
